...
Specialist clinics are under pressure: demand has grown, urgent patients wait too long, and session time cannot be expanded quickly. This report builds a simple and repeatable way to adjust appointment templates so we use the sessions we already have more effectively. The pipeline is three parts: first we screen all 26 clinics with five signals to find where a change would help the most; next we estimate short-term demand and compare it with typical weekly supply; finally we test a few realistic template settings and show what they mean for waitlists.

Using de-identified data, we reconstruct weekly counts by priority and visit type, then run scenario tests that lift access for urgent new patients while still protecting reviews. For clinic 2078B we show three options: keep urgent new at seventeen per week, lift it to nineteen, or lift it to twenty-four. We simulate backlog paths for three hundred weeks starting from the current waitlist. The results are clear: the first option holds steady but does not reduce the urgent list, the second clears it in roughly four years with moderate pressure on semi-urgent and routine, and the third clears it in about fifteen months but needs extra care to avoid spill-over. The outcome is a set of decisions the clinic can actually use, plus a simple way to monitor progress and adjust settings over time.

# Background & Motivation {.unnumbered}


Motivation. Public specialist clinics operate with persistent demand that exceeds short-term capacity. Session time is largely fixed by staffing, rosters, rooms, and funding cycles. As a result, throughput cannot be lifted quickly by adding clinics. At the same time, historical booking practice did not enforce clear rules for the mix of new and review visits across priority tiers. This looseness produced a gradual mismatch between capacity and need, most visible in prolonged waits for urgent new referrals. The practical question is not how to create more time in the near term, but how to use the existing time more effectively.

Research question. How can appointment templates be re-balanced under fixed weekly supply so that urgent waiting times fall, while continuity of care is preserved for semi-urgent and routine cohorts? The study seeks a policy that is operationally feasible, auditable, and explainable to clinical leads and schedulers.

Context and rationale. Referral volumes and the priority mix have shifted in the post-pandemic period. Legacy templates that once worked now under-serve urgent new patients. Clinics also face external performance expectations around timely access and equity across cohorts. A structured method is needed to identify where reallocation will help most, to quantify near-term pressure on each class, and to test realistic template settings before any change is rolled out.
...
Figure @fig-newreview shows attended activity by clinic, split into New and Review. When New is persistently under-allocated in high-volume clinics, urgent new referrals accumulate and first-appointment waits lengthen. The aim is to detect this structural imbalance early and target clinics for template reallocation.
...
The modelling uses monthly referral data from July 2023 to September 2025. To ensure an unbiased evaluation of forecast performance, the data are divided into two parts:
...
### Demand–Supply gap and why optimisation is needed

Having estimated the weekly demand for clinic 2078B and benchmarked it against the average weekly supply derived from FY2025 attended activity, it becomes clear that demand consistently exceeds available capacity. This persistent shortfall helps explain the extended wait times experienced by urgent patients, even though overall session time has remained stable. The underlying issue is not a lack of clinical effort, but rather an imbalance in the allocation of appointment types within the existing template. Too few slots are currently reserved for Urgent–New patients, while a substantial share continues to be occupied by follow-up reviews.

Because clinician session time is effectively fixed in the short term, expanding total capacity is not a feasible option. The more practical approach is to reconfigure the existing template mix, redistributing appointments among categories to better match the actual pattern of need. The goal is to increase capacity for Urgent–New patients while maintaining reasonable access for Semi-urgent and Routine cohorts. In this way, the clinic can improve timeliness for the most critical cases without undermining continuity of care for ongoing patients.
...
While maximising the weekly *service value*, we enforce the following requirements requested by the Council Team to ensure feasibility and fairness.
...
When we increase the weekly target for Urgent–New from 17 to 24 slots, the model directs more capacity to urgent patients, which is exactly what we expect. However, because total weekly supply stays fixed, this shift slightly reduces the space available for other new patients. As a result, the blue bars for SN and RN remain visible across all three settings — their unmet demand persists.
...
This project was conducted in two main phases.
